...
public class CourseController {
    ...
    public List<CourseModel> getStudentCourseAll(@PathVariable String studentNr) {
        ...
        for (GradeEntity gradeEntity : gradeEntityList) {
            ...
            courseModel.setType(gradeEntity.getGradeCompoundKey().getCourseEntity().getType());
    ...
    public String addCoursesByStudent(@RequestBody List<CourseModel> courseModelList, @PathVariable String studentNr) {
        ...
        for (CourseModel courseModel : courseModelList) {
            CourseEntity courseEntity = new CourseEntity(courseModel.getCode(), courseModel.getName(), courseModel.getEc(), courseModel.getType());
            ...
            if (gradeDAO.getByCompoundKey(gradeEntity) != null) {
...
/*    @RequestMapping(
            value = "add/course/{studentNr}",
            method= RequestMethod.POST
    )
    @ResponseBody
    public String addCoursesByStudent(@RequestBody CourseModel courseModel, @PathVariable String studentNr) {
        CourseDAO courseDAO = BeanHandler.getInstance().createCourseDAO();
        GradeDAO gradeDAO = BeanHandler.getInstance().createGradeDAO();
